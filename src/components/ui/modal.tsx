...
const ResponsiveModalVariants = cva(
  cn(
    'fixed z-50 gap-4 bg-white dark:bg-dark-card p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 overflow-y-auto',
...
>(({ side = 'bottom', className, children, ...props }, ref) => (
  <ResponsiveModalPortal>
    ...
    >
      {children}
      <ResponsiveModalClose className='absolute dark:text-primary-lightest right-4 top-4 rounded-sm opacity-70 ring-offset-primary dark:ring-offset-primary-lightest transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary'>
        <X className='h-4 w-4' />
